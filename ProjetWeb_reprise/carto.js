...
function fonction_blaireau(){
  ...
  if (sol_blaireau.checked && compteurb==0){
    ...
    message_blaireau.innerHTML='Bravo, comme ce volcan est haut!';
  }
}
...
function fonction_serpent(){
  ...
  if (sol_serpent.checked && compteurs==0){
    ...
    message_serpent.innerHTML="Bravo, même si c'est le parc le moins connu parmi les 4, c'est bien le parc des Great Smoky Mountains le plus visité des USA";
  }
}
...
function fonction_lion(){
  if (sol_lion.checked && compteurl==0){
    ...
    message_lion.innerHTML='Bravo, ça aurait été classe un combat contre un ours en vrai ';
  }
}
...
poudlard_express.on('dragend', function (e) {
   ...
   if (map.distance([lat_express,lng_express],[56.9745, -4.4125])<15000){
     ...
     message_hagrid2.innerHTML="<div>J'ai appris que tu voulais faire BCPST si tu avais pas été accepté à Poudlard. Si tu me trouves ces dragons dans leurs pays natal:</div>\
     <p>-Un boutefeu chinois </p>\
     <p>-Un suédois à museau court</p>\
     <p>-Un magyar à pointe (oui, le nom du pays est dans son nom) </p>\
     <p>-Un vert gallois</p>\
     <p>-Et mon cher Aragog qui est reparti en plein centre du pays qui possède la plus grande barrière de corail au monde</p>\
     <p> Je te fais membre à vie du club de zoologie magique</p>";
...
function fonction_vert_gallois(){
    if (sol_vert_gallois.checked && dragg==0){
      compteur_drag+=1;
        ...
        message_vert_gallois.innerHTML="Bravo";
      dragg=1;
    }
    if (compteur_drag==5){
      message_hagrid2.innerHTML=" Tu es décidément très doué, prend ce livre pour écrire toutes les observations que tu as pas avoir pendant ce voyage\
                      et va le faire signer par les directeurs de Durmstrang, dans l'Oural et Beauxbatons vers le parc du Morvan";
      overlay.style.display = 'block';
...
function fonction_suedois(){
    if (sol_suedois.checked && drags==0){
      compteur_drag+=1;
      $mdp.innerHTML=compteur_drag+'/5' ;
      message_suedois.innerHTML="Bravo";
      drags+=1;
    }
    if (compteur_drag==5){
      message_hagrid2.innerHTML=" Tu es décidément très doué, prend ce livre pour écrire toutes les observations que tu as pu avoir pendant ce voyage\
                      et va le faire signer par les directeurs de Durmstrang, dans l'Oural et Beauxbatons vers le parc du Morvan ";
      overlay.style.display = 'block';
      map.removeLayer(troisième_couche);
    couche3=0;
    ...
    if (sol_magyar.checked && dragh==0){
      compteur_drag+=1
      $mdp.innerHTML=compteur_drag+'/5' ;
      message_magyar.innerHTML="Bravo";
      dragh+=1;
    }
    if (compteur_drag==5){
      message_hagrid2.innerHTML=" Tu es décidément très doué, prend ce livre pour écrire toutes les observations que tu as pu avoir pendant ce voyage\
    ...
    if (sol_boutefeu.checked && dragc==0){
      compteur_drag+=1;
      $mdp.innerHTML=compteur_drag+'/5' ;
      message_boutefeu.innerHTML="Bravo";
      dragc+=1;
    }
    if (compteur_drag==5){
      message_hagrid2.innerHTML=" Tu es décidément très doué, prend ce livre pour écrire toutes les observations que tu as pu avoir pendant ce voyage\
...
map.on('zoomend', function() {
    if (map.getZoom() <8){
            ...
            map.removeLayer(cinquième_couche);
    }
    else {
      if (couche1==1){;
